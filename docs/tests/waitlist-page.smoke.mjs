...
test('waitlist page loads the early access form', async (t) => {
  ...
  await page.getByRole('heading', { name: /talk to us if your agents take real actions/i }).waitFor();
  await page.getByLabel(/name/i).waitFor();
  await page.getByLabel(/work email/i).waitFor();
  const demoHref = await page.getByRole('link', { name: /see the demo first/i }).getAttribute('href');
