"""OpenAI Agents SDK integration for Attest.

Intercepts every agent handoff via the official ``RunHooks.on_handoff`` API,
automatically issues a delegated Attest credential for the receiving agent,
...
Usage::
    ...
    from attest.integrations.openai_agents import AttestContext, AttestRunHooks

    @dataclass
    class AppContext(AttestContext):
        user_id: str = ""

    client = AttestClient(base_url="http://localhost:8080", api_key="dev")
    ...
    #     token = ctx.context.attest_tokens.get("billing-agent")
    #     ...  or use @attest_tool_openai decorator below
"""
...
from attest.types import DelegateParams, AttestClaims
...
# ---------------------------------------------------------------------------
# AttestContext — mixin dataclass for OpenAI Agents SDK contexts
# ---------------------------------------------------------------------------
...
class AttestContext:
    ...
    attest_tokens: dict[str, str] = field(default_factory=dict)

    # The root JWT issued before the run starts.
    # AttestRunHooks reads this as the parent when the first handoff fires.
    attest_root_token: str = ""

    # att_tid shared across the entire delegation tree.
    attest_task_id: str | None = None


# ---------------------------------------------------------------------------
# AttestRunHooks — the interceptor
# ---------------------------------------------------------------------------


class AttestRunHooks:
    """RunHooks subclass that issues delegated Attest credentials on every handoff.

    Pass an instance to ``Runner.run(hooks=...)``::

        result = await Runner.run(agent, input="...", context=ctx, hooks=AttestRunHooks(client))

    On each handoff from agent A → agent B, this hook:

    1. Determines B's scope from ``scope_map`` or auto-derives from ``to_agent.tools``.
    2. Finds A's current credential (or ``attest_root_token`` as fallback).
    3. Issues a delegated credential for B, bounded by A's scope.
    4. Stores the child JWT in ``context.context.attest_tokens[to_agent.name]``.

    Parameters
    ----------
    client:
        A sync ``AttestClient``. Called from async context via ``asyncio.to_thread``.
    scope_map:
    ...
    async def on_handoff(
        self,
        context: RunContextWrapper[AttestContext],  # type: ignore[type-arg]
        ...
        ctx: AttestContext = context.context  # type: ignore[attr-defined]

        # Find the parent credential: from_agent's token or root token.
        parent_token = ctx.attest_tokens.get(from_agent.name) or ctx.attest_root_token
        if not parent_token:
            raise RuntimeError(
                f"AttestRunHooks: no credential found for '{from_agent.name}' "
                "and attest_root_token is not set on the context. "
        ...
        ctx.attest_tokens[to_agent.name] = dt.token
        if ctx.attest_task_id is None:
            ctx.attest_task_id = dt.claims.att_tid
    ...
    def _resolve_scope(self, agent: Agent) -> list[str]:  # type: ignore[type-arg]
        ...
        2. ``agent.attest_scope`` attribute (set by developer on the Agent object).
        3. Auto-derive from tool names: ``tool:{tool.name}``.
        """
        if agent.name in self._scope_map:
            return self._scope_map[agent.name]

        if hasattr(agent, "attest_scope") and isinstance(agent.attest_scope, list):
            return agent.attest_scope  # type: ignore[return-value]
...
# ---------------------------------------------------------------------------
# attest_tool_openai — scope enforcement decorator for OpenAI Agents tools
# ---------------------------------------------------------------------------


def attest_tool_openai(scope: str, agent_name: str | None = None) -> Any:
    """Decorator that enforces Attest scope on an OpenAI Agents SDK tool function.
    ...
    agent_name:
        Optional explicit agent name to look up in ``context.context.attest_tokens``.
        If omitted, the decorator looks for the single credential in the context.

    Example::

        @function_tool
        @attest_tool_openai(scope="tool:send_email", agent_name="email-agent")
...
def _check_openai_scope(
    ...
    """Raise AttestScopeError if the credential in ctx does not cover required_scope."""
    import jwt as _jwt  # lazy import

    attest_ctx: AttestContext = ctx.context
    tokens = attest_ctx.attest_tokens
    ...
    if explicit_agent_name:
        ...
        raise RuntimeError(
            f"attest_tool_openai on '{tool_name}': multiple agent credentials present. "
            "Pass agent_name= to @attest_tool_openai."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"attest_tool_openai: failed to decode credential for '{resolved_name}': {exc}"
        ) from exc
